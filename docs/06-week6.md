...
In the top right we have the Environment, which is going to store objects, variables, datasets in the environment. We will get more into what an object is further into the chapter. 
...
This will add to the Environment (top right window) a new object called graduation date, which is equal to 2010. Instead of the assignment operator ``<-``, we can also use ``=``. 
...
In terms of data analaysis, we can really think of vectors as a variable. Our first variable was graduation date, our second variable was the school. Often, in data analysis, we want to compute some summary statistics of our variables. In R, there are a host of functions that are helpful in summarizing numeric vectors:
...
There are also a few ways to summarize information about the data frame. For example ``ncol()``, retrieves the number of columns. ``nrow()`` retrieves the number of columns. In this example, there are 5 variables, so there will be 5 columns. There are 4,870 observations, so there are 4,780 rows. One thing we can do to quickly get a sense of the data is to type ``summary(resume)``. This provides some simple summary statistics for each variable in the data frame. 
...
There is also another format for data that is common in R: "RData". You can save your file as an RData file by using the ``save()`` function.
...
Now that we've written out the different logical operators, let's see how to ask logical statements in R, and save logical objects. To begin, let's start with a simple logical statement: "5 is equal to 6". The way we type this in R is we type ``5==6``. This statement is of course false, so if we type this into R it should yield ``FALSE``.
...
Now let's try to understand the ``&`` operator. We use the ``&`` operator when we want to test if two statements are **both** true. For example, let's imagine we have an individual who is 18 and lives in California. Then the statement "This individual is 18 and lives in California" is true. The way we might type this in R is ``age==18 & state=="CA"``. We can do the same with testing numbers, as we have done before. For example, the statement "5 is equal to 5 and 5 is equal to 6" should be false.
...
In contrast, the statement "5 is equal to 5 and 5 is greater than 4" is true.
...
Here we have using indexing. ``[resume$race="black"]`` subsets to Black applicants. Because we have put this inside ``mean(resume$call[])``, then R will retrieve the average of the ``resume$call`` vector, but restrict to Black applicants. As we can see, about 6.4 percent of Black applicants receive a callback. 
